/**
 * FASTLY VCL EXAMPLES - FASTLY-SPECIFIC FUNCTIONS
 * 
...
/**
 * FUNCTION: fastly.hash
 * 
 * PURPOSE: Returns a hash value of the string key, using seed, and returning a number between from and to, inclusive
 * SYNTAX: fastly.hash(STRING key, INTEGER seed, INTEGER from, INTEGER to)
 * 
 * PARAMETERS:
 ...
 * 
 * RETURN VALUE: An integer between from and to, inclusive
 * 
 * NOTE: The underlying hash function might not offer cryptographic properties or collision resistance guarantees
 */
...
sub vcl_recv {
  ...
  # EXAMPLE 2: Consistent hashing for A/B testing
  ...
  # EXAMPLE 3: Load balancing with weighted distribution
  ...
  # EXAMPLE 4: Feature flagging system
...
/**
 * FUNCTION: fastly.ff.last_hop_was_serviceid
 * 
 * PURPOSE: Determines if the last hop in a Fastly-Fastly request chain was from the specified service ID
 * SYNTAX: fastly.ff.last_hop_was_serviceid(STRING service_id)
 * 
 * PARAMETERS:
...
sub vcl_recv {
  # EXAMPLE 1: Basic service chaining verification
  ...
  # EXAMPLE 2: Multi-service architecture with service chaining
...
/**
 * FUNCTION: fastly.try_select_shield
 * 
 * PURPOSE: Attempts to select a shield POP based on the client's location
 * SYNTAX: fastly.try_select_shield(STRING shield_name)
 * 
 * PARAMETERS:
...
sub vcl_recv {
  # EXAMPLE 1: Basic shield selection
  ...
  # EXAMPLE 2: Advanced shield selection with fallbacks
 ...
 * FUNCTION: h2.push
 * 
 * PURPOSE: Triggers an HTTP/2 server push of the asset at the specified path
 * SYNTAX: h2.push(STRING path)
 * 
 * PARAMETERS:
...
sub vcl_deliver {
  # EXAMPLE 1: Basic HTTP/2 server push
  ...
  # EXAMPLE 2: Conditional HTTP/2 server push based on client capabilities
  ...
  # EXAMPLE 3: Resource prioritization with HTTP/2 server push
...
/**
 * FUNCTION: h2.disable_header_compression
 * 
 * PURPOSE: Disables HTTP/2 header compression for the current request/response
 * SYNTAX: h2.disable_header_compression()
 * 
 * PARAMETERS: None
 * 
 * RETURN VALUE: None
...
sub vcl_deliver {
  # EXAMPLE: Disabling HTTP/2 header compression for security-sensitive contexts
 ...
 * FUNCTION: h3.alt_svc
 * 
 * PURPOSE: Adds an Alt-Svc header to advertise HTTP/3 support
 * SYNTAX: h3.alt_svc(STRING origin, INTEGER max_age)
 * 
 * PARAMETERS:
...
sub vcl_deliver {
  # EXAMPLE: Advertising HTTP/3 support
...
/**
 * FUNCTION: early_hints
 * 
 * PURPOSE: Sends an HTTP 103 Early Hints response with Link headers
 * SYNTAX: early_hints(STRING link_header)
 * 
 * PARAMETERS:
...
sub vcl_recv {
  # EXAMPLE 1: Basic Early Hints usage
  ...
  # EXAMPLE 2: Page-specific Early Hints
  ...
  # EXAMPLE 3: Combining Early Hints with HTTP/2 Server Push
 ...
 * 
 * This example demonstrates how multiple Fastly-specific functions can work together
 * to create a comprehensive performance optimization system.
 */
